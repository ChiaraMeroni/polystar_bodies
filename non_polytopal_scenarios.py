import numpy as np
import math
from radial_fns import add_plot_arbitrary_radial_func_3D
...
#Define the radial functions of the bodies we are interested in
#Radial function of the "Dented Tin Can" [Gardner]
def evaluate_radial_can(point):
    #Given a point in the ambient space evaluate the radial function
    z = point[2]
    if np.arccos(np.pi / 4) < z <= 1:
        numerator = (1 - z**2 + z * (-1 + np.sqrt(np.maximum(0, -1 + 2 * z**2))))
        denominator = (z * (-1 + z) * (z**2 - np.sqrt(np.maximum(0, -1 + 2 * z**2))))
        if np.isclose(denominator, 0):  # Check if denominator is effectively zero
            return 0
        return np.sqrt(numerator / denominator) #/ np.sqrt(np.pi)
    elif -np.arccos(np.pi / 4) < z < np.arccos(np.pi / 4):
        return 1 /np.sqrt(1 - z**2) #*1/np.sqrt(np.pi) 
    elif -1 <= z <= -np.arccos(np.pi / 4):
        numerator = (-1 + z**2 + z * (-1 + np.sqrt(np.maximum(0, -1 + 2 * z**2))))
        denominator = (z * (1 + z) * (-z**2 + np.sqrt(np.maximum(0, -1 + 2 * z**2))))
        if np.isclose(denominator, 0):  # Check if denominator is effectively zero
            return 0
        ...
        return 0  # Outside of defined range

# Radial function of the cylinder
def evaluate_radial_cyl(point):
    #Given a point in the ambient space evaluate the radial function
    z = point[2]
    if np.arccos(np.pi / 4)**2 < z**2 <= 1:
        denominator = np.sqrt(np.maximum(-z, z))
        return 1 / denominator #*1/ np.sqrt(np.pi)
    elif z**2 < np.arccos(np.pi / 4)**2:
        return 1 / np.sqrt(1 - z**2) #*1/np.sqrt(np.pi)
    else:
        return 0  # Outside of defined range
        
# Radial function of the elliptope
def evaluate_radial_elliptope(point):
    x = point[0]
    y = point[1]
    z = point[2]
    term1 = -1 + 54 * x**2 * y**2 * z**2
    term2 = 6j * np.sqrt(3) * np.sqrt(np.maximum(0,x**2 * y**2 * z**2 * (1 - 27 * x**2 * y**2 * z**2)))
    theta = np.angle(term1 + term2)
    if x * y * z < 0:
        return (1 / (6 * x * y * z)) * (np.sqrt(3) * np.sin(theta / 3) - np.cos(theta / 3) - 1)
    elif x * y * z > 0:
        return (1 / (6 * x * y * z)) * (2 * np.cos(theta / 3) - 1)
    else:
        return 1
...
if __name__ == "__main__":
    ...
    #Main parameter: up to which degree do we consider Fourier coefficients?
    total_degree = 20
    #Change scenario to see other examples
    scenario = "IBElliptope"
    assert scenario in ["DentedTinCan", "Cylinder", "Elliptope", "IBDentedTinCan", "IBCylinder", "IBElliptope"]


    if scenario == "DentedTinCan":
        #Scenario: Fourier approximation of the dented tin can [Gardner]:
        radial_fn_values = np.array([evaluate_radial_can(unit_vector) for unit_vector in roots])
        ...
        with open('data30can.json', 'w') as json_file: 
        ...
        ax.set(xticklabels=[],
            yticklabels=[],
            zticklabels=[])
        add_plot_arbitrary_radial_func_3D(expansion_function_evaluator, ax=ax, numpoints = 3000, color ="blue")
        # plt.savefig("DentedTinCan_"+str(total_degree)+".png")
        plt.show()


    if scenario == "Cylinder":
        #Scenario: Fourier approximation of the cylinder:
        radial_fn_values = np.array([evaluate_radial_cyl(unit_vector) for unit_vector in roots])
        ...
        with open('data30cyl.json', 'w') as json_file: 
            json.dump(total_dict, json_file, indent=4) 
        ...
        ax.set(xticklabels=[],
            ...
            zticklabels=[])
        add_plot_arbitrary_radial_func_3D(expansion_function_evaluator, ax=ax, numpoints = 3000, color ="blue")
        # plt.savefig("Cyl_"+str(total_degree)+".png")
        plt.show()


    if scenario == "Elliptope":
        #Scenario: Fourier approximation of the elliptope:
        radial_fn_values = np.array([evaluate_radial_elliptope(unit_vector) for unit_vector in roots])
        ...
        with open('data30ell.json', 'w') as json_file: 
            json.dump(total_dict, json_file, indent=4) 
        ...
        ax.set(xticklabels=[],
            ...
            zticklabels=[])
        add_plot_arbitrary_radial_func_3D(expansion_function_evaluator, ax=ax, numpoints = 3000, color ="blue")
        # plt.savefig("Elliptope_"+str(total_degree)+".png")
        plt.show()


    if scenario=="IBDentedTinCan":
        #Scenario: Weighted approximation of the intersection body of the dented tin can

        radial_fn_values = np.array([(evaluate_radial_can(unit_vector)**2)/2 for unit_vector in roots])
        #Next we want to compute the harmonic expansion up to a given target degree
        normalization_coefficients_dict = compute_normalization_coefficients_dict(total_degree)
        spherical_harmonics_values_dict = compute_spherical_harmonic_values_at_point_array(roots, normalization_coefficients_dict, total_degree)
        #Once the evaluations have been computed we can do the integration to obtain Fourier coefficients via our quadrature
        ...
        with open('dataIB30can.json', 'w') as json_file: 
            json.dump(total_dict, json_file, indent=4) 

        #Now we do a picture with the result
        ...
        ax.set(xticklabels=[],
            ...
            zticklabels=[])
        add_plot_arbitrary_radial_func_3D(expansion_function_evaluator, ax=ax, numpoints = 3000, color ="blue")
        # plt.savefig("IB_DentedTinCan_"+str(total_degree)+".png")
        plt.show()


    if scenario=="IBCylinder":
        #Scenario: Weighted approximation of the intersection body of the cylinder

        radial_fn_values = np.array([(evaluate_radial_cyl(unit_vector)**2)/2 for unit_vector in roots])
        #Next we want to compute the harmonic expansion up to a given target degree
        normalization_coefficients_dict = compute_normalization_coefficients_dict(total_degree)
        spherical_harmonics_values_dict = compute_spherical_harmonic_values_at_point_array(roots, normalization_coefficients_dict, total_degree)
        #Once the evaluations have been computed we can do the integration to obtain Fourier coefficients via our quadrature
        ...
        with open('dataIB30cyl.json', 'w') as json_file: 
            json.dump(total_dict, json_file, indent=4) 
        ...
        ax.set(xticklabels=[],
            ...
            zticklabels=[])
        add_plot_arbitrary_radial_func_3D(expansion_function_evaluator, ax=ax, numpoints = 3000, color ="blue")
        # plt.savefig("IB_Cylinder_"+str(total_degree)+".png")
        plt.show()


    if scenario=="IBElliptope":
        #Scenario: Weighted approximation of the intersection body of the elliptope

        radial_fn_values = np.array([(evaluate_radial_elliptope(unit_vector)**2)/2 for unit_vector in roots])
        #Next we want to compute the harmonic expansion up to a given target degree
        normalization_coefficients_dict = compute_normalization_coefficients_dict(total_degree)
        spherical_harmonics_values_dict = compute_spherical_harmonic_values_at_point_array(roots, normalization_coefficients_dict, total_degree)
        #Once the evaluations have been computed we can do the integration to obtain Fourier coefficients via our quadrature
        ...
        with open('dataIB30ell.json', 'w') as json_file: 
            json.dump(total_dict, json_file, indent=4) 
        ...
        ax.set(xticklabels=[],
            ...
            zticklabels=[])
        add_plot_arbitrary_radial_func_3D(expansion_function_evaluator, ax=ax, numpoints = 3000, color ="blue")
        # plt.savefig("IB_Elliptope_"+str(total_degree)+".png")
        plt.show()
